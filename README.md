...
#### Класс BasketItem
Класс представляет один товар, находящийся в корзине. Он является частью списка товаров, который отображается в корзине. Этот класс наследуется от Card (то есть, использует функциональность для отображения информации о товаре) и добавляет для корзины функции, такие как отображение индекса товара в корзине и кнопку удаления товара из корзины.
